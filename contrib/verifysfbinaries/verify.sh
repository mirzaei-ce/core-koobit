...
###   This script attempts to download the signature file SHA256SUMS.asc from koobit.org
...
WORKINGDIR="/tmp/koobit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://koobit.org/bin/"
VERSIONPREFIX="koobit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'koobit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Koobit Core binary release signing key installed?"
   fi
