...
#ifdef ENABLE_WALLET
        // Case 1: Koobit address and we have full public key:
        CKoobitAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    ...
    CKoobitAddress address(innerID);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"koobitaddress\" \"signature\" \"message\"\n"
...
{
    if (type == 2) {
        address = CKoobitAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CKoobitAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CKoobitAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
