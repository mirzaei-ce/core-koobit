Koobit Core version 0.11.0 is now available from:

  <https://koobit.org/bin/koobit-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Koobit Core or other software:
...
defaults to 0.00001. This will cause transactions with fewer KBT/kB fee to be
...
For example, add the following to `koobit.conf`:
...
related to the blockchain in the koobit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Koobit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Koobit Core will 
...
For an example script see [koobit-submittx](https://github.com/laanwj/koobit-submittx).
...
- #5334 `cf87536` libkoobitconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new koobit-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only KoobitAmountField
- #5753 `0f44672` Add koobit logo to about screen
...
- #5809 `46bfbe7` Add koobit-cli man page
...
- #5849 `41259ca` contrib/init/koobitd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Koobit-Qt.app to Koobit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for koobit-tx)
